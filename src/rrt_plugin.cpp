...
public:
                          ...
                          const std::string& parameterNamespace) override
    {
        for (const std::string& gpName : model->getJointModelGroupNames())
        {
            ...
            RRTPlanningContextPtr(new RRTPlanningContext("rrt_planning_context", gpName, model, node));
        }
        return true;
    }
    ...
    planning_interface::PlanningContextPtr getPlanningContext(const planning_scene::PlanningSceneConstPtr& planningScene,
                                                  ...
                                                  moveit_msgs::msg::MoveItErrorCodes& errorCode) const override
    {
        ...
        std::cout << "context getGroupName: " << context->getGroupName() << " getName: " << context->getName() << std::endl;
