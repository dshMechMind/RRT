...
int main(int argc, char **argv)
{
    ...
    pluginlib::ClassLoader<planning_interface::PlannerManager> polyLoader("moveit_core", "planning_interface::PlannerManager");
    planning_interface::PlannerManagerPtr rrtPlanner = polyLoader.createSharedInstance("rrt_interface/RRTPlanner"); // chomp_interface/CHOMPPlanner ompl_interface/OMPLPlanner
    ...
    moveit_visual_tools::MoveItVisualTools visualTools(rrtDemoNode, "panda_link0",
                                                        "move_group_tutorial", robotModel); // move_group.getRobotModel()
    ...
    std::shared_ptr<rclcpp::Publisher<moveit_msgs::msg::DisplayTrajectory>> displayPublisher =
        rrtDemoNode->create_publisher<moveit_msgs::msg::DisplayTrajectory>("/display_planned_path", 1);
    ...
    std::vector<double> goalValues = {-1.0, 0.7, 0.7, -1.5, -0.7, 2.0, 0.0}; // -0.4 -0.325 0.36 -1.996 -0.38 1.991 0.425 
    ...
    rrt_interface::RRTPlanningContextPtr rrtContext = std::static_pointer_cast<rrt_interface::RRTPlanningContext>(context);
    visualTools.trigger();
    displayPublisher->publish(displayTrajectory);

    /* We will add more goals. But first, set the state in the planning
      scene to the final state of the last plan */
    robotState->setJointGroupPositions(jmg, response.trajectory.joint_trajectory.points.back().positions);
